...
namespace Storage
{
                ...
                try
                {
                    lock (this.Clients)
                    {
                        ...
                        for (int i = 0; i < Clients.Length; i++)
                        {
                            ...
                            {
                                if (Clients[i].InactiveTime >= 2000) // Not used in the last %x% seconds
                                {
                    ...
                    for (uint i = 0; i < Clients.Length; i++)
                    {
                        if (AvailableClients[i] == true)
                        {
                            ...
                            if (Clients[i].State == ConnectionState.Closed)
                            {
                                ...
                                {
                                    Console.WriteLine("Errore di connessione al database: " + e.Message);
                                }
                            }
    ...
    class DatabaseClient : IDisposable
    {
        ...
        public void Connect()
        {
            ...
            {
                throw new DatabaseException("[DBClient.Connect]: Impossibile aprire la connessione MySQL.");
            }
        }
